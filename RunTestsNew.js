...
class RepositoryManager {
  ...
  toggleElementsVisibility(elements, className) {
    elements.forEach(id => document.getElementById(id).classList.toggle(className));
  }
  ...
  createFileElement(fileName, filePath) {
    ...
    fileElement.addEventListener("click", () => this.changeTitleAndHide(fileName, filePath));
    return fileElement;
  }

  createDirectoryElement(directory) {
    const directoryElement = document.createElement("div");
    const directoryContentElement = this.createDirectoryContentElement(directory.contents);
  ...
  createDirectoryContentElement(contents) {
    ...
    directoryContentElement.className = "border-l-2 border-gray-200 pl-2 hidden";

    contents.forEach(content => {
      if (content.type === "file") {
        directoryContentElement.appendChild(this.createFileElement(this.extractFileNameFromPath(content.path), content.path));
      }
      if (content.type === "dir") {
        directoryContentElement.appendChild(this.createDirectoryElement(content));
      }
    });
  ...
  createSpanElement(path) {
    ...
    spanElement.className = "flex items-center px-4 py-2 text-sm cursor-pointer select-none";

    const fileNameElement = document.createElement("span");
    fileNameElement.textContent = this.extractFileNameFromPath(path);
    fileNameElement.addEventListener("click", () => this.changeTitleAndHide(this.extractFileNameFromPath(path)));

    spanElement.appendChild(fileNameElement);
    spanElement.innerHTML += ' <i data-feather="chevron-down" class="ml-1 w-4 h-4 transform"></i>';
  ...
  async changeTitleAndHide(title, filePath) {
    ...
    this.toggleElementsVisibility([SELECTORS.fileDropdown, SELECTORS.searchBar, SELECTORS.createFileBtn], HIDDEN_CLASS);
    document.getElementById(SELECTORS.filePath).textContent = filePath;

    const fileContents = await this.fetchFileContents();
    document.getElementById("codeBox").innerHTML = Prism.highlight(fileContents.file_content, Prism.languages.python, "python");
  }
